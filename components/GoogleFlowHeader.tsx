...
export default function GoogleFlowHeader() {
  const pathname = usePathname();

  const steps = [
    { id: 1, label: "Cadastro do Tutor", href: "/auth/register-google" },
    { id: 2, label: "Cadastro do Pet", href: "/auth/pet-register" },
  ];
  ...
  return (
    <div className="w-full bg-[#0a0f0d] border-b border-hairline-border py-3 px-4 sticky top-0 z-[100] backdrop-blur-md">
      <div className="max-w-7xl mx-auto flex items-center justify-start md:justify-center overflow-x-auto no-scrollbar gap-2 md:gap-4">
        <div className="flex items-center gap-2 mr-4 text-xs font-bold text-primary bg-primary/10 px-3 py-1.5 rounded-full border border-primary/20 shrink-0">
        ...
        {steps.map((step) => {
          ...
          return (
            <Link
              key={step.id}
              href={step.href}
              className={`px-4 py-2 rounded-full text-xs font-bold transition-all shrink-0 flex items-center gap-1.5 ${isActive
